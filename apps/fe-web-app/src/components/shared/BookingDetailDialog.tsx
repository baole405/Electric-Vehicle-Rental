...
export default function BookingDetailDialog({
  ...
  const paymentSummary =
    ...
    (booking.payment && typeof booking.payment === 'object'
      ? (booking.payment as TPayment)
      : undefined);

  const relatedRental =
    rental ??
    (booking.rental && typeof booking.rental === 'object'
      ? (booking.rental as TRental)
      : undefined);
  ...
  const showApprovalButtons = isPendingApproval && (onApprove || onReject);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-2 text-sm">
              ...
              {booking.reservationExpiresAt && (
                <div className="md:col-span-2">
                  ...
                  <div className="font-medium">
                    {fmt(booking.reservationExpiresAt)}
                  </div>
                </div>
              )}
            ...
            <div className="mt-3 grid gap-2 text-sm">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="font-medium">
                  {money(pricing?.surchargeAmount ?? pricing?.additionalFees)}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-muted-foreground">Deposit</span>
                <span className="font-medium">
                  {money(pricing?.depositAmount)}
                </span>
              </div>
              <Separator />
              <div className="flex items-center justify-between text-base font-semibold">
                ...
                <span>
                  {money(pricing?.totalPayable ?? pricing?.totalRentalFee)}
                </span>
              </div>
            </div>
          </section>
              ...
              <div className="mt-3 grid gap-3 md:grid-cols-2 text-sm">
                ...
                <div>
                  ...
                  <div className="font-mono text-xs">
                    {paymentSummary.txnRef ?? 'N/A'}
                  </div>
                </div>
                <div>
                  <div className="text-muted-foreground">Total</div>
                  <div className="font-medium">
                    {money(paymentSummary.totalAmount)}
                  </div>
                </div>
              </div>
            </section>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">
                        {fmt(entry.updatedAt ?? entry.createdAt)}
                      </div>
                      {entry.note && (
            ...
            <section className="rounded-lg border p-4">
              ...
              <div className="mt-3 grid gap-3 md:grid-cols-2 text-sm">
                ...
                <div>
                  ...
                  <div className="font-medium">
                    {fmt(relatedRental.pickupTime)}
                  </div>
                </div>
                <div>
                  <div className="text-muted-foreground">Return time</div>
                  <div className="font-medium">
                    {fmt(relatedRental.returnTime ?? undefined)}
                  </div>
                </div>
        ...
        {/* Action buttons for PENDING_APPROVAL status */}
        {showApprovalButtons && (
