...
export function StaffBookingManagement() {
  const { useBookingList, confirmBooking, updateBookingStatus } = useBooking();
  ...
  const rentals = useMemo(
    () =>
      Array.isArray(rentalQuery.data?.data?.data)
  ...
  const handleConfirmBooking = async (booking: TBooking) => {
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : 'Không thể xác nhận booking.'
      );
    }
  };
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-gray-50">
                    <TableHead className="min-w-[120px]">Mã Booking</TableHead>
                    <TableHead className="min-w-[120px]">Khách hàng</TableHead>
                    <TableHead className="min-w-[150px]">Liên hệ</TableHead>
                    <TableHead className="min-w-[150px]">Dòng xe</TableHead>
                    <TableHead className="min-w-[120px]">Thời gian</TableHead>
                    <TableHead className="min-w-[110px]">Tổng tiền</TableHead>
                    <TableHead className="min-w-[130px]">Trạng thái</TableHead>
                    <TableHead className="text-right min-w-[180px] sticky right-0 bg-gray-50">
                      Thao tác
                    </TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {filteredBookings.map((booking) => {
                    ...
                    // Terminal statuses (no actions allowed)
                    ...
                    // Check if booking is pending approval (UPPERCASE or lowercase)
                    ...
                    // Check if paid but waiting for rental creation
                    const isPaidWaitingRental =
                      booking.status === 'PAID' && !rental;
                    ...
                    return (
                      <TableRow key={booking._id} className="hover:bg-gray-50">
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            {booking.brand?.imageUrl && (
                              <img
                                src={booking.brand.imageUrl}
                                alt={booking.brand.name}
                          ...
                          <div className="flex flex-wrap items-center justify-end gap-2">
                            ...
                            {!isTerminalStatus && (
                              <>
                                ...
                                {/* WAITING_PAYMENT: Manual payment confirmation (if bank transfer) */}
                                {isWaitingPayment && showManualPayment && (
                                  <Button
                                    variant="outline"
                                    size="sm"
                                    onClick={() =>
                                ...
                                {/* PAID: Show waiting message (BGJ creating rental) */}
                                {isPaidWaitingRental && (
                                ...
                                {/* SUCCESS: Show handover buttons if rental exists */}
                                {rental && booking.status === 'SUCCESS' && (
                                  <>
                                    <Button
                                      variant="outline"
                                      size="sm"
                                      onClick={() =>
                                        handleOpenHandoverDialog(
                                          rental,
                                          'pickup'
                                        )
                                      }
                                ...
                                {/* Assign vehicle if not assigned */}
                                ...
                                {/* Update status button (for advanced actions) */}
                                <Button
                                  variant="ghost"
                                  size="sm"
                                  onClick={() => handleUpdateStatus(booking)}
