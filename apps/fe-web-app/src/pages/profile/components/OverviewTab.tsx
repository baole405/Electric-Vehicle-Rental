...
import { cn, fmt, mapStatusColor, money, statusText } from '@/lib/utils';
...
export default function OverviewTab({ onTabChange }: OverviewTabProps) {
  ...
  const bookingStats = useMemo(() => {
    ...
    const waiting = bookings.filter((booking) =>
      WAITING_SET.has(booking.status?.toUpperCase() ?? '')
    ).length;
    const active = bookings.filter((booking) => {
      const status = booking.status?.toUpperCase() ?? '';
      return (
        !COMPLETED_SET.has(status) &&
        status !== 'CANCELLED' &&
        status !== 'EXPIRED'
      );
    }).length;
    const completed = bookings.filter((booking) =>
      COMPLETED_SET.has(booking.status?.toUpperCase() ?? '')
    ).length;
    const totalSpent = bookings
      ...
      .reduce(
        (acc, booking) =>
          acc + (booking.pricing?.totalPayable ?? booking.totalPayable ?? 0),
        0
      );
  ...
  const statsCards = [
    {
      ...
      icon: Calendar,
      trend: bookingStats.waiting
        ? `Chờ thanh toán: ${bookingStats.waiting}`
        : undefined,
    },
    {
      ...
      trend: bookings.length
        ? `${Math.round((bookingStats.active / bookings.length) * 100)}%`
        : undefined,
    },
    {
      title: 'Hoàn tất',
      ...
      icon: CheckCircle2,
      trend: bookings.length
        ? `${Math.round((bookingStats.completed / bookings.length) * 100)}%`
        : undefined,
    },
    {
      title: 'Tổng chi tiêu',
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Ba booking mới nhất của bạn và trạng thái hiện tại.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {bookings.length === 0 ? (
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Trạng thái hồ sơ & giấy tờ đã gửi.
            </CardDescription>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Thông tin nhanh về giao nhận và giao dịch gần đây.
            </CardDescription>
          </CardHeader>
