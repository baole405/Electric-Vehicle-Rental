...
export default function VehicleDetailPage() {
  ...
  const canBook = Boolean(
    vehicle && vehicle.status === 'available' && isVerified
  );
  ...
  const handleBookVehicle = () => {
    ...
    if (!isVerified) {
      toast.warning(
        'Tài khoản chưa được xác minh giấy tờ. Hoàn tất hồ sơ để đặt xe.'
      );
          ...
          <div className="mt-6 flex flex-col gap-3">
            ...
            {!isVerified && (
              <p className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-2 text-sm text-amber-700">
                T�i kho?n c?a b?n ch?a ???c x�c minh gi?y t?. Vui l�ng ho�n t?t
                h? so tr??c khi d?t xe.
              </p>
            )}
            <p className="text-xs text-gray-500 leading-relaxed">
              *S? li?u ph?m vi/xang s?c l� u?c lu?ng minh ho?. Xe th?t c� th?
              kh�c t�y c?u h�nh & di?u ki?n s? d?ng.
            </p>
          </div>
        </div>
      </div>
