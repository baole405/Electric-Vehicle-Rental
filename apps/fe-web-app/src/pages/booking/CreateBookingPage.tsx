...
import { CreateBookingFormSchema, convertFormToBookingAPI } from "@/schema/booking.schema";
...
export default function CreateBookingPage() {
  ...
  const handleChange = (field: keyof TCreateBookingForm, value: string | boolean) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      err.errors?.forEach((error) => {
        ...
        if (field === 'agreedToPaymentTerms') {
          newFieldErrors.agreedToPaymentTerms = "Bạn phải đọc và đồng ý với điều khoản thanh toán để tiếp tục";
        } else if (field === 'agreedToDataSharing') {
          newFieldErrors.agreedToDataSharing = "Bạn phải đồng ý với điều khoản chia sẻ dữ liệu cá nhân để tiếp tục";
      ...
      if (Object.keys(newFieldErrors).length > 0) {
        const fieldOrder = ['renterName', 'phoneNumber', 'email', 'pickupDate', 'pickupTime', 'returnDate', 'returnTime', 'agreedToPaymentTerms', 'agreedToDataSharing'];
        const firstErrorField = fieldOrder.find(field => newFieldErrors[field]);

        if (firstErrorField) {
          const element = document.getElementById(firstErrorField) || document.querySelector(`[data-terms-section]`);
          if (element) {
    ...
    console.log("  - Final vehicleId:", state?.vehicle?._id || cleanedData.vehicleId);
    ...
    try {
      ...
      console.warn("Technical error during data conversion - check form validation");
      return;
    }
    ...
    createBooking.mutate(apiData, {
      ...
      onError: (error: unknown) => {
        ...
        console.error("  - Request Data was:", JSON.stringify(apiData, null, 2));
        ...
        if (err?.response?.status === 400) {
          ...
          errorMessages.unshift("❓ Không tìm thấy tài nguyên (404 Not Found):");
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        if (fieldErrors.renterName) {
                          setFieldErrors(prev => ({ ...prev, renterName: '' }));
                        }
                      }}
                      placeholder="Nguyễn Văn A"
                      className={`h-12 ${fieldErrors.renterName ? 'border-red-500 focus:border-red-500' : ''}`}
                    />
                    {fieldErrors.renterName && (
                      <div className="text-xs text-red-600 bg-red-50 p-2 rounded border border-red-200">
                        <span role="img" aria-label="error">⚠️</span> {fieldErrors.renterName}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      className={`h-12 ${fieldErrors.phoneNumber ? 'border-red-500 focus:border-red-500' : ''}`}
                    />
                    {fieldErrors.phoneNumber && (
                      <div className="text-xs text-red-600 bg-red-50 p-2 rounded border border-red-200">
                        <span role="img" aria-label="error">⚠️</span> {fieldErrors.phoneNumber}
                      </div>
                    )}
                  ...
                  <div className="space-y-2">
                    ...
                    {fieldErrors.email && (
                      <div className="text-xs text-red-600 bg-red-50 p-2 rounded border border-red-200">
                        <span role="img" aria-label="error">⚠️</span> {fieldErrors.email}
                      </div>
                    )}
                  </div>
                </div>
              ...
              {/* Notes Field */}
              <div className="bg-white rounded-2xl shadow-md p-6">
                <h2 className="text-lg font-bold text-gray-900 mb-4">Ghi chú (Tùy chọn)</h2>
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    rows={4}
                  />
                  <p className="text-xs text-gray-500">Mọi yêu cầu đặc biệt về thời gian, địa điểm nhận xe, hoặc thông tin khác</p>
                </div>
              </div>

              {/* Terms & Conditions */}
              <div className="bg-white rounded-2xl shadow-md p-6 space-y-4" data-terms-section>
                <div className="space-y-2">
                  <div className="flex items-start gap-3">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => {
                        ...
                        if (checked) {
                          setFieldErrors(prev => ({ ...prev, agreedToPaymentTerms: '' }));
                        }
                      }}
                      className={`mt-1 ${!formData.agreedToPaymentTerms && fieldErrors.agreedToPaymentTerms ? 'border-red-500' : ''}`}
                    />
                  ...
                  {fieldErrors.agreedToPaymentTerms && (
                    <div className="ml-8 text-xs text-red-600 bg-red-50 p-2 rounded border border-red-200">
                      <span role="img" aria-label="error">⚠️</span> {fieldErrors.agreedToPaymentTerms}
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-start gap-3">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => {
                        ...
                        if (checked) {
                          setFieldErrors(prev => ({ ...prev, agreedToDataSharing: '' }));
                        }
                      }}
                      className={`mt-1 ${!formData.agreedToDataSharing && fieldErrors.agreedToDataSharing ? 'border-red-500' : ''}`}
                    />
                  ...
                  {fieldErrors.agreedToDataSharing && (
                    <div className="ml-8 text-xs text-red-600 bg-red-50 p-2 rounded border border-red-200">
                      <span role="img" aria-label="error">⚠️</span> {fieldErrors.agreedToDataSharing}
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl shadow-md p-6 sticky top-4">
                  ...
                  <div>
                    ...
                    {state?.vehicle && (
                      <p className="text-sm font-medium text-blue-600 mt-1">
                        <span role="img" aria-label="car">🚗</span> {state.vehicle.plateNo || "Chưa có biển số"}
                      </p>
                    )}
                  </div>
                </div>
                ...
                {state?.vehicle && (
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                    <h4 className="text-sm font-semibold text-blue-900 mb-3">Thông tin xe được chọn</h4>
                    <div className="grid grid-cols-2 gap-3 text-xs">
                      <div className="flex items-center gap-2">
                        <span className="text-blue-600" role="img" aria-label="battery">🔋</span>
                        <div>
                          <p className="text-blue-700">Pin</p>
                          <p className={`font-medium ${state.vehicle.batteryPercent >= 70 ? 'text-green-600' :
                            state.vehicle.batteryPercent >= 30 ? 'text-yellow-600' : 'text-red-600'
                            }`}>
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-blue-600" role="img" aria-label="odometer">⚙️</span>
                        <div>
                          <p className="text-blue-700">ODO</p>
                          <p className="font-medium text-blue-900">
                            {state.vehicle.odometer ? `${state.vehicle.odometer.toLocaleString()}km` : 'N/A'}
                          </p>
                        </div>
                      </div>

                      <div className="flex items-center gap-2">
                        <span className="text-blue-600" role="img" aria-label="vehicle-id">🏷️</span>
                        <div>
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-blue-600" role="img" aria-label="location">📍</span>
                        <div>
                ...
                <div className="bg-gray-50 rounded-lg p-4 mb-4 border border-gray-200">
                  <div className="flex items-center justify-between mb-3">
                    <p className="text-sm font-semibold text-gray-700">Thời gian thuê</p>
                    <button
                      type="button"
                    ...
                    <div className="space-y-3">
                      ...
                      <div>
                        <label className="text-xs font-medium text-gray-600 block mb-1">
                          <span role="img" aria-label="calendar">📅</span> Ngày & giờ nhận xe
                        </label>
                        <div className="grid grid-cols-2 gap-2">
                          <div className="space-y-1">
                            <Input
                              ...
                              className={`text-xs h-8 ${fieldErrors.pickupDate ? 'border-red-500 focus:border-red-500' : ''}`}
                              min={today}
                            />
                            {fieldErrors.pickupDate && (
                              <div className="text-xs text-red-600 bg-red-50 p-1 rounded border border-red-200">
                                <span role="img" aria-label="error">⚠️</span> {fieldErrors.pickupDate}
                              </div>
                            )}
                          </div>
                          <div className="space-y-1">
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                if (fieldErrors.pickupTime) {
                                  setFieldErrors(prev => ({ ...prev, pickupTime: '' }));
                                }
                              }}
                              className={`text-xs h-8 ${fieldErrors.pickupTime ? 'border-red-500 focus:border-red-500' : ''}`}
                            />
                            {fieldErrors.pickupTime && (
                              <div className="text-xs text-red-600 bg-red-50 p-1 rounded border border-red-200">
                                <span role="img" aria-label="error">⚠️</span> {fieldErrors.pickupTime}
                              </div>
                            )}
                          </div>
                        </div>
                      ...
                      <div>
                        <label className="text-xs font-medium text-gray-600 block mb-1">
                          <span role="img" aria-label="return">🔄</span> Ngày & giờ trả xe
                        </label>
                        <div className="grid grid-cols-2 gap-2">
                          <div className="space-y-1">
                            <Input
                              ...
                              className={`text-xs h-8 ${fieldErrors.returnDate ? 'border-red-500 focus:border-red-500' : ''}`}
                              min={formData.pickupDate}
                            />
                            {fieldErrors.returnDate && (
                              <div className="text-xs text-red-600 bg-red-50 p-1 rounded border border-red-200">
                                <span role="img" aria-label="error">⚠️</span> {fieldErrors.returnDate}
                              </div>
                            )}
                          </div>
                          <div className="space-y-1">
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                if (fieldErrors.returnTime) {
                                  setFieldErrors(prev => ({ ...prev, returnTime: '' }));
                                }
                              }}
                              className={`text-xs h-8 ${fieldErrors.returnTime ? 'border-red-500 focus:border-red-500' : ''}`}
                            />
                            {fieldErrors.returnTime && (
                              <div className="text-xs text-red-600 bg-red-50 p-1 rounded border border-red-200">
                                <span role="img" aria-label="error">⚠️</span> {fieldErrors.returnTime}
                              </div>
                            )}
                          </div>
                        </div>
                      ...
                      <div className="text-xs text-gray-500 bg-blue-50 p-2 rounded">
                        <span className="font-medium">{pricing.rentalDays} ngày</span> thuê • Hình thức: Theo ngày
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="border-t pt-4 space-y-4">
                  <h3 className="font-bold text-gray-900 mb-3">
                    <span role="img" aria-label="receipt">🧾</span> Bảng kê chi tiết
                  </h3>
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-sm">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-blue-900 font-semibold">
                        {state?.brand?.baseDailyRate?.toLocaleString("vi-VN") || "0"}đ/ngày
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between items-center py-2">
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          {state?.brand?.baseDailyRate?.toLocaleString("vi-VN") || "0"}đ × {pricing.rentalDays} ngày
                        </p>
                      </div>
                    ...
                    {pricing.additionalFees > 0 && (
                      <div className="flex justify-between items-center py-2">
                        <div>
                          <span className="text-amber-700">Phụ phí cuối tuần</span>
                          <p className="text-xs text-amber-600">
                            100.000đ × {Math.floor(pricing.additionalFees / 100000)} ngày cuối tuần
                          </p>
                        </div>
                    ...
                    <div className="border-t border-gray-200 pt-3">
                      <div className="flex justify-between items-center py-2">
                        <span className="font-semibold text-gray-900">Tổng tiền thuê xe</span>
                      ...
                      <div className="flex justify-between items-center py-2">
                        <div>
                          ...
                          <p className="text-xs text-gray-500">Hoàn trả sau khi trả xe</p>
                        </div>
                        <span className="font-semibold text-gray-900">
                  ...
                  <div className="bg-green-50 border-2 border-green-300 rounded-lg p-4 mt-4">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-lg font-bold text-green-800">Thanh toán ngay</span>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[80vh] overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b bg-green-50">
              <h2 className="text-xl font-bold text-gray-900">Điều khoản thanh toán</h2>
              <button
            ...
            <div className="p-6 overflow-y-auto max-h-[60vh] space-y-4">
              <div className="prose prose-sm max-w-none">
                <h3 className="text-lg font-semibold text-gray-900 mb-3">1. Phương thức thanh toán</h3>
                <ul className="list-disc list-inside space-y-2 text-gray-700">
                  <li>Khách hàng thanh toán qua chuyển khoản ngân hàng theo thông tin được cung cấp</li>
                  <li>Thanh toán phải được thực hiện sau khi booking được nhân viên duyệt</li>
                  <li>Thời hạn thanh toán: trong vòng 24 giờ kể từ khi nhận thông báo duyệt</li>
                </ul>

                <h3 className="text-lg font-semibold text-gray-900 mb-3 mt-6">2. Tiền đặt cọc và hoàn trả</h3>
                <ul className="list-disc list-inside space-y-2 text-gray-700">
                  <li>Tiền cọc sẽ được hoàn trả sau khi kết thúc thuê xe và kiểm tra tình trạng xe</li>
                  <li>Nếu xe bị hư hại, chi phí sửa chữa sẽ được trừ vào tiền cọc</li>
                  <li>Thời gian hoàn cọc: 3-5 ngày làm việc sau khi trả xe</li>
                </ul>

                <h3 className="text-lg font-semibold text-gray-900 mb-3 mt-6">3. Chính sách hủy booking</h3>
                <ul className="list-disc list-inside space-y-2 text-gray-700">
                  <li>Hủy trước 24h: Hoàn 100% tiền đã thanh toán</li>
                  <li>Hủy trong 24h: Chỉ hoàn 50% tiền thuê xe, không hoàn tiền cọc</li>
                  <li>Không xuất hiện: Không hoàn tiền</li>
                </ul>

                <h3 className="text-lg font-semibold text-gray-900 mb-3 mt-6">4. Trách nhiệm của khách hàng</h3>
                <ul className="list-disc list-inside space-y-2 text-gray-700">
                ...
                <h3 className="text-lg font-semibold text-gray-900 mb-3 mt-6">5. Bảo hiểm và trách nhiệm</h3>
                <ul className="list-disc list-inside space-y-2 text-gray-700">
                  <li>Xe được bảo hiểm bắt buộc và bảo hiểm vật chất</li>
                  <li>Khách hàng chịu trách nhiệm về tài sản cá nhân trong xe</li>
                  <li>Trong trường hợp tai nạn, khách hàng phải thông báo ngay cho công ty</li>
                </ul>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[80vh] overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b bg-blue-50">
              <h2 className="text-xl font-bold text-gray-900">Điều khoản chia sẻ dữ liệu cá nhân</h2>
              <button
            ...
            <div className="p-6 overflow-y-auto max-h-[60vh] space-y-4">
              <div className="prose prose-sm max-w-none">
                <h3 className="text-lg font-semibold text-gray-900 mb-3">1. Thu thập thông tin</h3>
                <ul className="list-disc list-inside space-y-2 text-gray-700">
                  <li>Họ tên, số điện thoại, email để liên lạc và xử lý booking</li>
                ...
                <h3 className="text-lg font-semibold text-gray-900 mb-3 mt-6">2. Mục đích sử dụng</h3>
                <ul className="list-disc list-inside space-y-2 text-gray-700">
                ...
                <h3 className="text-lg font-semibold text-gray-900 mb-3 mt-6">3. Bảo mật thông tin</h3>
                <ul className="list-disc list-inside space-y-2 text-gray-700">
                ...
                <h3 className="text-lg font-semibold text-gray-900 mb-3 mt-6">4. Quyền của khách hàng</h3>
                <ul className="list-disc list-inside space-y-2 text-gray-700">
                ...
                <h3 className="text-lg font-semibold text-gray-900 mb-3 mt-6">5. Thời gian lưu trữ</h3>
                <ul className="list-disc list-inside space-y-2 text-gray-700">
                  <li>Thông tin booking: 5 năm (theo quy định pháp luật)</li>
                  <li>Thông tin liên lạc: Cho đến khi khách hàng yêu cầu xóa</li>
                  <li>Dữ liệu phân tích: Được ẩn danh hóa sau 2 năm</li>
                </ul>

                <h3 className="text-lg font-semibold text-gray-900 mb-3 mt-6">6. Liên hệ</h3>
