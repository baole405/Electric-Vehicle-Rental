        ...
        <nav className="flex flex-1 flex-col gap-1 text-sm">
          {DASHBOARD_MENU.filter((item) => item.roles.includes(effectiveRole)).map((item) => (
            <NavLink
              ...
              className={({ isActive }) =>
                cn(
                  "rounded-lg px-3 py-2 font-medium transition hover:bg-primary/10",
      ...
      <main className="flex flex-1 flex-col">
        <header className="flex items-center justify-between border-b bg-white px-4 py-4 shadow-sm md:px-8">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={() => navigate(ROUTES.ROOT)}>
              <Menu className="mr-2 h-4 w-4" /> Home
            </Button>
        ...
        <div className="mx-auto w-full max-w-6xl flex-1 px-4 py-6 md:px-8 md:py-10">{children}</div>
      </main>
    </div>
  );
};
